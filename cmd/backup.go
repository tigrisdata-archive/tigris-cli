...
var backupCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		withLogin(cmd.Context(), func(ctx context.Context) error {
			util.Stdoutf(" [o] using timeout %s\n", backupTimeout)
			os.Setenv("TIGRIS_TIMEOUT", backupTimeout)
			...
			for _, db := range databases {
				...
				filename := fmt.Sprintf("./%s.schema", db)
				...
				for _, collection := range collections {
					...
					// Dump data
					filename = fmt.Sprintf("./%s.%s.json", db, collection)
					var doc driver.Document
					for it.Next(&doc) {
						err := os.WriteFile(filename, []byte(string(doc)), 0o600)
...
func init() {
	...
	backupCmd.Flags().StringVarP(&backupTimeout, "timeout", "t", "1h",
		"timeout specification (e.g. 1h)")
